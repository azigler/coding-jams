...
for (const edge of edges) {
  ...
  const wasConnected = uf.find(edge.i) === uf.find(edge.j)
  if (uf.union(edge.i, edge.j)) {
    connectionsMade++
    console.log(`${connectionsMade}. Connect Box ${edge.i} <-> Box ${edge.j} (dist: ${edge.dist.toFixed(2)})`)
  } else {
    console.log(`   Skip: Box ${edge.i} <-> Box ${edge.j} (already in same circuit)`)
  }
}
