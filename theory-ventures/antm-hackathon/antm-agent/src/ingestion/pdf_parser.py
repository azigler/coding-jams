"""PDF parser for batch processing PDFs to markdown."""
...
class PDFParser:
    ...
    def images_to_markdown(self, images: List[dict]) -> str:
        ...
        for img in images:
            relative_path = Path(img["path"]).relative_to(self.output_dir)
            markdown_images.append(
...
def parse_pdf_batch(
    ...
    """
    Parse all PDFs from multiple directories.

    Args:
        pdf_dirs: List of directories containing PDFs
        output_dir: Output directory for parsed markdown files
    ...
    if progress:
        pdf_files = tqdm(pdf_files, desc="Parsing PDFs")

    for pdf_file in pdf_files:
        try:
            parser = PDFParser(str(pdf_file), str(output_dir))
            markdown_content = parser.parse()
            parser.save_output(markdown_content)
            parser.close()
            stats["success"] += 1
