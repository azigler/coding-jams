...
tracked in git, and every meaningful state is already committed via
`Bead:` trailers -- git history IS the authoritative snapshot history.

The beads-rust `.beads/.gitignore` ships with patterns for the sqlite
DB + lock files but **does NOT include `.br_history/`** as of this
writing (upstream oversight). Result: every session shows
`?? .beads/.br_history/` in `git status`, creating persistent noise
and the risk of accidentally `git add -A`-staging it.
